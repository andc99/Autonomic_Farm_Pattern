...
class Collector: public ProcessingElement{
	private:
		...
		void body(){
			...
			while(eos_counter < this->n_buffers){ //if has been received EOSs as many the workers, stop 
				this->start_time = std::chrono::high_resolution_clock::now();
				this->next_pop(&task);
				eos_counter += (task == EOS) ? 1 : 0;
				ssize_t &t = (*((ssize_t*) task));
				this->collector_buffer->safe_push(&t); 
				if ( (act_context_id = this->get_context() ) != sched_getcpu()) //check if has to change its context
		...
		//clousure to encapsulate the pop from workers buffers
